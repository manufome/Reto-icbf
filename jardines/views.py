...
def editar_jardin(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Actividad.objects.create(
                tipo='edicion',
                descripcion=f'Jardín editado: {jardin.nombre}. Campos modificados: {", ".join(cambios)}',
                icono='fa-school',
                content_type=ContentType.objects.get_for_model(jardin),
                object_id=jardin.id
            )
            if 'estado' in cambios:
                if jardin.estado in ['En trámite', 'Negado']:
                    messages.warning(request, f'El jardín {jardin.nombre} ha sido puesto bajo supervisión con estado {jardin.estado}')
                else:
                    messages.success(request, f'El jardín {jardin.nombre} ahora tiene el estado {jardin.estado}')
            else:
                messages.success(request, f'Jardín {jardin.nombre} actualizado exitosamente. Campos modificados: {", ".join(cambios)}')
            return redirect('lista_jardines')
    else:
